...
- **File:** `README.md`
- **Introduce a “Breaking changes in 1.0.0” section near the top:** detail encoder change (example showing `"+"` transforming to `%2B`) and new enforcement with opt-out instructions.
- **Under `cookie.set` documentation:** mention that passing `SameSite: "None"` now throws unless `secure: true` or `allowInsecureSameSiteNone` is configured.
- **Update examples:** show `cookie.configure({ allowInsecureSameSiteNone: true })` for local dev, and emphasise that production should always provide `secure: true`.
- **Add migration guide snippet:** instruct how to decode values if consumers relied on legacy `unescape` behavior.
